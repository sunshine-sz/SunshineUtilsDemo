...
public class DynamicWave extends View {
    ...
    private static final String TAG = DynamicWave.class.getSimpleName();
    ...
    public DynamicWave(Context context, AttributeSet attrs) {
        ...
        mXOffsetSpeedOne = ConvertUtils.dp2px(context, TRANSLATE_X_SPEED_ONE);
        mXOffsetSpeedTwo = ConvertUtils.dp2px(context, TRANSLATE_X_SPEED_TWO);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mTotalWidth; i++) {
            ...
            // 减200只是为了控制波纹绘制的y的在屏幕的位置，大家可以改成一个变量，然后动态改变这个变量，从而形成波纹上升下降效果
            // 绘制第一条水波纹
    ...
    private void resetPositonY() {
        ...
        //将源数组复制到新数组(mYPositions 源数组；mXTwoOffset 源数组开始位置；mResetTwoYPositions 目标数组； yTwoInterval 要复制的数组的元素数量)
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        Logger.e(TAG,"onSizeChanged"+w+"--"+h);
        // 记录下view的宽高
        mTotalWidth = w;
        mTotalHeight = h;
        // 用于保存原始波纹的y值
